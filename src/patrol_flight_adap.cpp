...
geometry_msgs::Twist Adaptive()
{
	...
	{*/
		...
		if (fly_s)
		{
			...
			/*if(dist>0)
			{
				speedpid.linear.y =0.2*adaptiveDist(dist);//最大0.01
			}
			else
			{
				speedpid.linear.y =0.1*adaptiveDist(dist);//最大0.01
			}*/
			speedpid.linear.y =0.0;
			//speedpid.linear.y=0.0;
            speedpid.linear.z =adaptive(e_height);
			if(angle>0)
			{
			  double temn=2*adaptiveYaw(angle);
			  if(temn>1.0)
			  {
			...
			{
				speedpid.angular.z =2*adaptiveYaw(angle);
			}
			cout<<"无人机正在巡航"<<endl;
		}
		//调整态，平移和旋转
		else 
		{
			...
			if(angle>0)
			{
			  double temn1=4*adaptiveYaw(angle);
			  if(temn1>1.0)
			  {
				speedpid.angular.z =1.0;
			  }
			...
			{
				double temn2=4*adaptiveYaw(angle);
...
void flight_test_()
{
	ros::NodeHandle video_;   //节点句柄
	 ...
	 //static ros::Publisher move_pub; //移动话题发布者
	 //初始化 移动话题发布者
						 ...
						 switch(t.code)
						 {
								   ...
								   case KEY_DOWN_DOWN:
								   {
									...
									cout<<"后退"<<endl;
								 	...
								 	case KEY_D:
								   {
									speedadjust.angular.z=-0.5;
									 cout<<"右转"<<endl;
									break;
								   }
									case KEY_A:
								   {
									 speedadjust.angular.z=1.0;
									 cout<<"左转"<<endl;
									 break;
								   }
								  case KEY_W:
