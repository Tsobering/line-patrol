...
bool sta_s=false;//稳定态,此处有修改，之前为true
bool adj_s=false;
bool bigadj_s=false;
int aa=0;
int bb=0;
...
//飞行合理角度
float flight_angle=0.3;    //单位是弧度？
float flight_dist=160;     //单位是mm？
//创建两个窗口
...
class pid
{
	...
	public:
		...
		pid(float kp_,float ki_,float speedmax_,float speedmin_,float errmax_)
		{
			Setvalue=0.0;
			Actualvalue=0.0;
			err=0.0;
	...
	//此函数仅用于偏航角控制
		float PID_realize(float error)    //调用函数所赋值即为偏差值
		{
			   int index;
			   err_last=err;
			   err=error;
if(speed>speedmax)
		{
if(fabs(err)>errmax)     //当静差大于17度取消积分作用
{
	index=0;
}
else
...
speed=Kp*err+index*Ki*integral;//+Kd*(err-err_last);

cout << "积分量 integral= ：" << integral << endl;
...
pid pid_dist(0.00023,0.000009,0.015,-0.015,150.0);    //像素宽度是640,640x0
//std::cout<<"dist kp"<<pid_dist.get_kp()<<std::endl;
//pid_dist.set_kp(0.0015);
pid pid_angle(0.75,0.005,0.1275,-0.1275,0.15);     //像素宽度是640,640x90度的弧度即1.57等于1，即偏转的最大速度
//pid_angle.set_kp(0.636);
pid pid_angle_1(0.35,0.0,0.1275,-0.1275,0.15);  //激进的左右控制,有点太激进，可尝试减小
...
{	
	ros::NodeHandle video_n;   //节点句柄
	ros::Publisher takeoff_pub; //起飞话题发布者
	takeoff_pub=video_n.advertise<std_msgs::Empty>("ardrone/takeoff", 1);//将节点设置成发布者，并将所发布主题和类型的名称告知节点管理器，
	//缓存区设置为1个消息      ardrone/takeoff是否是驱动程序留的接口？？？？是的  在chatter topic中发布一个std_msgs/Empty类型的数据，让master告诉nodes听取chatter，在这个topic上我们将发布数据
	ros::Rate loop_rate(10);
	std_msgs::Empty empty_off;   //创建消息变量，变量的类型需要符合发送数据的要求
	...
	while(ros::ok())
	{
		...
		if(second<5)
		{	//发布起飞话题，开始起飞
			takeoff_pub.publish(empty_off);      //驱动节点直接订阅消息？
...
void land_()
{
	ros::NodeHandle video_n;   //节点句柄
	 ros::Publisher land_pub;   //着陆话题发布者
	 //初始化 着陆话题发布者
   	land_pub=video_n.advertise<std_msgs::Empty>("ardrone/land",1);           //驱动程序订阅的
...
geometry_msgs::Twist adaptive(double target_height,double m_height)
{
	...
	geometry_msgs::Twist speed;    //消息变量？
...
//自适应定高飞行回调函数？
void callback(const ardrone_autonomy::NavdataConstPtr & nav)  //navdata constptr意思？，包含头文件和主题所使用的消息类型
{
		...
		float alt = nav->altd;       //navdata为驱动节点发布的消息类型？
	...
	//下面这段什么意思？
	        ros::Duration(0.02).sleep();  //睡眠0.02s，每次运行到这里都要睡眠0.02s，是否会影响反应时间
	        speedadaptive.linear.z = 0.0;  //为什么在这里要把高度设为0？
	        cmd_vel_pub_ada.publish(speedadaptive);
...
int Rising_heighted_()
{
  	 ...
  	 //	rh_cmd_vel_pub=video_n.advertise<geometry_msgs::Twist>("cmd_vel",1);
		...
		rh_navdata_sub=video_n.subscribe<ardrone_autonomy::Navdata>("ardrone/navdata",1,callback);   //尖括号内为订阅的数据类型
	//自适应定高？
	
		ros::Rate loop_rate(20);  //设定消息调用频率，每0.05s读取一次数据？
...
void Stable()
{
	...
	int second=0;
	geometry_msgs::Twist speed;
    speed.linear.x=0.0;          //speed是驱动节点的一个类？是ros里面定义的
    speed.linear.y=0.0;
	speed.linear.z=0.0;
...
void PathDetection(const Mat& image_row)
{
	...
	{
		dist_=dist - dist_last;
		if(fabs(dist)>fabs(dist_last))
		{
			ee=true;
		}
		else
...
//当距离大于80且距离变换率较大时采用激进的PID控制
		//if(fabs(angle) < 0.15 && fabs(dist) > 70 && fabs(dist_) > 8 && fabs(dist) < flight_dist && ee)
		//{
         // fly_s = false;
		  //adj_s=false;
		  //bigadj_s=true;
...
//if(fabs(angle) < 0.15 && fabs(dist) > flight_dist )
//		{
//          fly_s = false;
//		  adj_s=true;
//		  bigadj_s=false;
...
//		}
		...
		//返回偏角
		cout << "偏角：" << angle * (180 / PI)<< endl;   //将弧度换算成角度
		cout << "距离：" << dist << endl;
	}
...
geometry_msgs::Twist PID()
{
	...
	if(sta_s)
	{
		speedpid.linear.x =0;
		speedpid.linear.y =0;
		speedpid.linear.z =0;   //此为自适应定高，不宜为0
		speedpid.angular.x=0;
		speedpid.angular.y=0;
	...
	else
	{
		...
		if (fly_s)
		{
			speedpid.linear.x = 0.09;//0.1
			
			speedpid.angular.z =pid_angle.PID_realize(angle);  //angle为图像处理函数计算得来？
			speedpid.linear.y =pid_dist.PID_realize(dist);
			//speedpid.linear.y =0;
			cout<<"无人机正在巡航"<<endl;
		}

//左右调整。在左右调整态应该将左右调整稳定后再进入飞行态，此段代码不完善，预计不会得到良好的控制效果，可去除
		if (adj_s)
		{
			speedpid.linear.x =0;//0.1
			
			speedpid.angular.z =pid_angle_1.PID_realize(angle);  //angle为图像处理函数计算得来？
			speedpid.linear.y =pid_dist.PID_realize(dist);
			cout<<"无人机正在左右移动"<<endl;
		}
...
if (bigadj_s)
		{
			...
			speedpid.angular.z =0;  //angle为图像处理函数计算得来？
			//speedpid.linear.y =pid_dist1.PID_realize(dist);
			cout<<"无人机变换率较大，正在左右移动"<<endl;
		}

		//调整态，平移和旋转
		if (!adj_s && !fly_s && !bigadj_s)
		{
			speedpid.linear.y =pid_dist.PID_realize(dist);//最大0.01       限幅是在哪里实现？
			speedpid.linear.x = 0;
	...
	cout << "角速度 z= ：" << speedpid.angular.z << endl;

	return speedpid;             //speedpid是在哪里定义？
